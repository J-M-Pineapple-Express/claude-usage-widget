...
- **Sessions**: running more than one Claude Code session? Click **sessions** (or **+2 more**) at the end of that line to see every running session (terminals, VS Code, Claude Desktop) with its context and transcript size, whether it's working or idle, and where it's running. Click one to pin it to the widget's context bar; otherwise the widget follows whichever session was active most recently.
