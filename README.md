...
- **Context**: how full your active Claude Code context window is, so you get a heads-up before it auto-compacts. Reads the most recently active transcript under `~/.claude/projects/` and updates every 15 s. Click the line under the bar to set your window size (200K / 500K / 1M); it measures against about 80% of that, where compaction kicks in. Green → yellow → red as you fill up. The line under the bar also shows the transcript's size (`jsonl 4.7MB`, green under 12MB, yellow under 15MB, red above), and the line below that shows which project and session it's measuring, for example `Claude Command Center · Projects`. **Hover that line** to see where you left off (Claude Code's latest recap for the session and the last thing you asked), or **click it** to open that in a window.
- **Sessions**: running more than one Claude Code session? Click **sessions** (or **+2 more**) at the end of that line to see every running session (terminals, VS Code, Claude Desktop) with its context and transcript size, whether it's working or idle, and where it's running. Hover the link for the same list as a tooltip. In the window, **where I left off** expands a session's recap in place. Click a session to pin it to the widget's context bar; otherwise the widget follows whichever session was active most recently. Bots and scripts that run Claude Code headless (`claude -p --resume`, like a Discord bot) get their own **Bots & scripts** section there, with message count, last reply and context.
- **Resets**: one-off usage resets from Anthropic, which refill your 5-hour and weekly limits when you use one. Shows how many you have and when they expire; click to use one on claude.ai. Hidden if your account isn't eligible.
- **Sections** fold: click a section's ▾ header to collapse it to a one-line summary. The widget sizes itself to whatever is open and remembers your choice.
